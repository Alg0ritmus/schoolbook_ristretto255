...
int main(){
	...
	u8 s_minus_1[32] = {0xec,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0x7f};

	// negative vectors
	u8 test_negative_vectors[8][32] = {
	...
	// P - negative vector
	u8 test_negative_vectors_compl[8][32] = {
	...
	for (int i = 0; i < 7; ++i){
		...
		result &= bytes_eq_32(bytes_out_, MAP_VECTORS_RESULT[i]); // returns 1 if two are eq

		#ifdef VERBOSE_FLAG
		if (!result){
				printf("HASH_TO_GROUP TEST no.%d: FAILED! Error was found when testing vector MAP_VECTORS[64][%d]\n",i,i);
		}
		else{
	...
	for (int i = 0; i < 8; ++i){
		...
		result &= is_neg(in); // returns 1 if two are eq
	...
	for (int i = 0; i < 8; ++i){
		...
		result &= 1-is_neg(in); // returns 1 if two are eq
	...
	for (int i = 0; i < 4; ++i)
	{
		result &= 1-ristretto255_decode(out_rist,non_canonical_vectors[i]);
	...
	for (int i = 0; i < 8; ++i)
	{
		result &= ristretto255_decode(out_rist,non_square_x2[i]);
	...
	for (int i = 0; i < 8; ++i)
	{
		result &= ristretto255_decode(out_rist,negative_xy[i]);
